...
>>>>>>> a15ae561d52746b4fd377fd78effafc2d4fff0ee
        ...
        <div class="main-content p-4">
            ...
            @if (isset($existingAssessmentInfo))
                <div class="container py-4" style="margin-top: 20px;">
                    @if ($existingAssessmentInfo['is_non_director'] ?? false)
                            ...
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    <h5 class="alert-heading mb-3">
                                        <i class="fas fa-clock me-2"></i>Assessment Already Completed
                                    </h5>
                                    <div class="mb-3">
                                        <p class="mb-2">
                                            You have already completed the assessment for
                                            <strong>{{ $existingAssessmentInfo['phc_name'] }}</strong>
                                            in <strong>{{ $existingAssessmentInfo['quarter'] }}
                                                {{ $existingAssessmentInfo['year'] }}</strong>.
                                        </p>
                                        <p class="mb-2">
                                            <strong>Edit window closed:</strong>
                                            <span
                                                class="text-muted">{{ $existingAssessmentInfo['edit_expired_at']->format('M j, Y g:i A') }}</span>
                                        </p>
                                        <p class="mb-0">
                                            <i class="fas fa-info-circle me-2"></i>
                                            <strong>What's next?</strong> Wait for the next quarter to open, or contact
                                            your administrator if you need to make changes to your completed assessment.
                                        </p>
                                    </div>
                    ...
                    @else
                        ...
                        <div class="alert alert-info border-0 shadow-sm">
                            <div class="d-flex align-items-start">
                                ...
                                <div class="flex-grow-1 ms-3">
                                    <h5 class="alert-heading mb-3">
                                        <i class="fas fa-clock me-2"></i>Assessment Period Already Completed
                                    </h5>
                                    <div class="mb-3">
                                        <p class="mb-2">
                                            An assessment for
                                            ...
                                            in <strong>{{ $existingAssessmentInfo['quarter'] }}
                                                {{ $existingAssessmentInfo['year'] }}</strong>
                                            has already been completed and the editing period has expired.
                                        </p>
                                        <p class="mb-0">
                                            <i class="fas fa-lightbulb me-2 text-warning"></i>
                                            <strong>Solution:</strong> Please select a different quarter or a different
                                            PHC to conduct a new assessment.
                                        </p>
                                    </div>

                                    <div class="mt-3">
                                        <button type="button" class="btn btn-primary" onclick="location.reload()">
                                            <i class="fas fa-plus me-2"></i>Select Different Location/Period
                                        </button>
...
>>>>>>> a15ae561d52746b4fd377fd78effafc2d4fff0ee
    ...
    {{-- @if (auth()->user()->role->name === 'director' && isset($showDirectorModal) && $showDirectorModal)
        @include('components.director-phc-modal-enhanced')
    @endif --}}

    @if (auth()->user()->role->name === 'director' && isset($showDirectorModal) && $showDirectorModal)
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const modalElement = document.getElementById('phcSelectionModal');
                if (modalElement) {
                    const phcModal = new bootstrap.Modal(modalElement, {
                        backdrop: 'static',
                        keyboard: false
                    });
                    phcModal.show();
